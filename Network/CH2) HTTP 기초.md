...
HTTP/1.0은 요청마다 TCP 연결을 처음부터 새로 맺었다. 파일이 수십 개씩 붙는 웹페이지에서 그 비용은 고스란히 로딩 시간에 나타났다. 연결을 재사용하는 Keep-Alive, 한 연결로 여러 요청을 동시에 처리하는 멀티플렉싱 — HTTP는 이 문제를 줄이는 방향으로 버전을 거듭했다.
